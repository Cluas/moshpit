...
final class TmuxScrollForensicsUITest: XCTestCase {
    ...
    func testScrollUpDownForensics() throws {
        ...
        let card = app.descendants(matching: .any)
            ...
            .firstMatch
        // A forensics driver has nothing to say on a device with no saved
        // connection (every CI simulator): skip rather than fail.
        try XCTSkipUnless(card.waitForExistence(timeout: 15),
                          "forensics driver: needs a saved connection on this device")
        shot(app, "01-home")
