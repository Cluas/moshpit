...
final class AppearanceFlowUITest: XCTestCase {
    ...
    func testIconGalleryOffersDistinctIconsAndSwitches() throws {
        ...
        for id in ["default", "pit", "island", "wq", "prefix", "localhost",
                   "prompt", "cursor", "hail"] {
            XCTAssertTrue(app.buttons["app-icon-\(id)"].exists, "missing icon option \(id)")
        }

        // Switching the icon must not disturb the accent. The simulator cannot
        // actually switch (lsd fails to load its confirmation-alert plugin —
        // POSIX 5/35, same for PNG-based alternates); a device can, and on a
        // success iOS shows a "you changed the icon" alert — dismiss it if it
        // appears so the run doesn't stall.
