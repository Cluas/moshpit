...
///
/// Gated on the same `MOSHPIT_SSH_USER` / `MOSHPIT_SSH_LOCAL_KEY` env vars as
...
struct GoldenPathTests {
    static var username: String? { ProcessInfo.processInfo.environment["MOSHPIT_SSH_USER"] }
    static var keyPath: String? { ProcessInfo.processInfo.environment["MOSHPIT_SSH_LOCAL_KEY"] }
    ...
    static let isConfigured: Bool = {
        ...
        if !configured {
            print("[GoldenPath] SKIPPED: set MOSHPIT_SSH_USER and MOSHPIT_SSH_LOCAL_KEY to run the golden-path integration suite.")
        }
        return configured
    }()
    ...
    func formSaveThenSSHConnect() async throws {
        ...
        // Confirm bytes flow.
        let marker = "moshpit-golden-\(UUID().uuidString.prefix(8))"
