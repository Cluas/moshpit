...
///
/// Gated on the same MOSHPIT_SSH_USER / MOSHPIT_SSH_LOCAL_KEY envvars as
...
struct TmuxIntegrationTests {
    nonisolated static let username: String? = ProcessInfo.processInfo.environment["MOSHPIT_SSH_USER"]
    nonisolated static let keyPath: String? = ProcessInfo.processInfo.environment["MOSHPIT_SSH_LOCAL_KEY"]
    ...
    nonisolated static let isConfigured: Bool = {
        ...
        if !configured {
            print("[TmuxIntegration] SKIPPED: set MOSHPIT_SSH_USER and MOSHPIT_SSH_LOCAL_KEY to run the tmux integration suite.")
        }
        return configured
    }()
    ...
    private static func uniqueSessionName() -> String {
        "moshpit-test-\(UUID().uuidString.prefix(8))"
    }
