...
struct SessionMetricsTests {
    ...
    private struct Fixture {
        let registry: SessionMetricsRegistry
        let store: ConnectionStore
        let defaults: UserDefaults
        let name: String
    }

    private func makeRegistryAndStore() -> Fixture {
        let name = "test.sessionmetrics.\(UUID().uuidString)"
        let defaults = UserDefaults(suiteName: name)!
        return Fixture(registry: SessionMetricsRegistry(), store: ConnectionStore(defaults: defaults),
                       defaults: defaults, name: name)
    }

    @Test("liveConnections returns only servers whose metrics report a live state")
    func liveConnectionsFilters() {
        let fixture = makeRegistryAndStore()
        let registry = fixture.registry, store = fixture.store, defaults = fixture.defaults, name = fixture.name
