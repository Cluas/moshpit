...
struct FeedCostTests {
    ...
    ///
    /// A shared CI runner's wall clock is not the parser's cost: the two-core
    /// GitHub host took 70 ms over what the simulator does in 26. The workflow
    /// marks itself (`TEST_RUNNER_MOSHPIT_CI`, which xcodebuild hands to the
    /// test process as `MOSHPIT_CI`) and the budget stretches there; the
    /// product number stays the local one.
    static var budgetMilliseconds: Double {
        ProcessInfo.processInfo.environment["MOSHPIT_CI"] == nil ? 60 : 180
    }
    ...
    /// The least of three runs: the parser's cost, not the scheduler's mood or
    /// the one-time tables the first feed pays to build.
    private func feedMilliseconds(lines count: Int) -> Double {
        (0..<3).map { _ in feedOnceMilliseconds(lines: count) }.min() ?? .infinity
    }

    private func feedOnceMilliseconds(lines count: Int) -> Double {
