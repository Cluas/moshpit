...
struct HerdrSocketClientTests {
    ...
    /// A reply hook the client's writer can call before the test has decided
    /// what the reply is: set `fn` once the client exists, no captured `var`.
    final class Hook: @unchecked Sendable {
        var fn: (@Sendable (String) async -> Void)?
        func call(_ line: String) async { await fn?(line) }
    }
    ...
    func requestEnvelopeShape() async throws {
        ...
        let client = HerdrSocketClient { data in
            if let id = server.record(data) {
                await respond.call(#"{"id":"\#(id)","result":{"type":"ok"}}"#)
            }
        }
        respond.fn = { line in await client.feed(Data((line + "\n").utf8)) }
    ...
    func serverErrorThrows() async throws {
        ...
        let client = HerdrSocketClient { data in
            if let id = server.record(data) {
                await respond.call(#"{"id":"\#(id)","error":{"code":"pane_not_found","message":"no such pane"}}"#)
            }
        }
        respond.fn = { line in await client.feed(Data((line + "\n").utf8)) }
    ...
    func subscriptionEncoding() async throws {
        ...
        let client = HerdrSocketClient { data in
            if let id = server.record(data) {
                await respond.call(#"{"id":"\#(id)","result":{"type":"subscription_started"}}"#)
            }
        }
        respond.fn = { line in await client.feed(Data((line + "\n").utf8)) }
