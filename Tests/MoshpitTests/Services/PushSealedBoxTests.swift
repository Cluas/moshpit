...
///
/// Format v1 is implemented three times — `scripts/host/moshpit-push.sh` (openssl, on
...
struct PushSealedBoxTests {
    ...
    /// The same secret the Go tests and scripts/verify/push-vector.sh use.
    ...
    /// `scripts/verify/push-vector.sh` and update BOTH this and
