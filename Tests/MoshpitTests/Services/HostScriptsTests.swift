...
///
/// Two copies of each script exist — the file under `scripts/host/` and a Swift
...
struct HostScriptsTests {
    ...
    /// checkout, so there a missing file is skipped rather than failed — the
    /// same guard also runs in `scripts/verify/verify-push-e2e.sh`, which
    /// always has it. On a simulator the checkout is always reachable, so a
    /// missing file can only mean the directory walk below is wrong, and it
    /// FAILS: when `Tests/` gained a level this returned nil, the guards below
    /// returned early, and both comparisons passed while comparing nothing.
    private func repoScript(_ name: String) throws -> String? {
        ...
        let url = root.appendingPathComponent("scripts/host/\(name)")
    ...
    @Test("the stamp script matches scripts/host/moshpit-stamp.sh")
    func stampMatchesRepo() throws {
        guard let onDisk = try repoScript("moshpit-stamp.sh") else { return }
        #expect(HostScripts.stamp == onDisk.trimmingCharacters(in: .newlines),
                "run scripts/gen/gen-host-scripts.py")
    }

    @Test("the sender matches scripts/host/moshpit-push.sh")
    func senderMatchesRepo() throws {
        guard let onDisk = try repoScript("moshpit-push.sh") else { return }
