...
final class MainFlowUITest: XCTestCase {
    ...
    func testMainFlowSmoke() throws {
        ...
        app.launchArguments += ["-RINGDOWN_RESET"]   // empty store → + always opens the form
        app.launch()

        // 1. Home: large "Attach" title
        XCTAssertTrue(app.staticTexts["Ringdown"].waitForExistence(timeout: 5),
                      "Ringdown title should be visible at launch")
        ...
        XCTAssertTrue(app.staticTexts["Ringdown"].waitForExistence(timeout: 3))
    ...
    func testAddAndDeleteConnection() throws {
        let app = XCUIApplication()
        app.launchArguments += ["-RINGDOWN_RESET"]   // empty store, under the free cap
        app.launch()
        XCTAssertTrue(app.staticTexts["Ringdown"].waitForExistence(timeout: 5))
