...
/// variable `RINGDOWN_SSH_LOCAL_KEY` pointing at an OpenSSH private key whose
/// public counterpart sits in `~/.ssh/authorized_keys`, and `RINGDOWN_SSH_USER`
...
///
/// Then:
///   - `export RINGDOWN_SSH_USER=$(whoami)`
///   - `export RINGDOWN_SSH_LOCAL_KEY=$HOME/.ssh/id_ed25519`
///
/// before invoking `xcodebuild test ... -only-testing:RingdownTests/SSHIntegration`.
@Suite("SSHIntegration", .tags(.integration), .serialized)
struct SSHIntegrationTests {
    static let username: String? = ProcessInfo.processInfo.environment["RINGDOWN_SSH_USER"]
    static let keyPath: String? = ProcessInfo.processInfo.environment["RINGDOWN_SSH_LOCAL_KEY"]
    ...
    static let isConfigured: Bool = {
        ...
        if !configured {
            print("[SSHIntegration] SKIPPED: set RINGDOWN_SSH_USER and RINGDOWN_SSH_LOCAL_KEY to run the SSH integration suite (see file header for sshd setup).")
        }
        return configured
    }()
