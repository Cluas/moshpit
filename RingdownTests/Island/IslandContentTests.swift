...
///   - the `@ringdown_title` → `Agent.detail` schema (backward-compatible decode)
...
struct IslandContentTests {
    ...
    func jsonCommand() {
        ...
        #expect(cmd.contains("ringdown-stamp.sh"))
        #expect(cmd.contains("@ringdown_title"))
        ...
        #expect(cmd.contains(".ringdown.bak."))
    ...
    func codexCommand() {
        ...
        #expect(cmd.contains("# >>> ringdown vibe island >>>"))
        #expect(cmd.contains("# <<< ringdown vibe island <<<"))
