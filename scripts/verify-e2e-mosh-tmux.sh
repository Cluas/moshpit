#!/usr/bin/env bash
# T4 — end-to-end self-verification of Offhook over REAL mosh+tmux to localhost.
#
...
# (OFFHOOK_SEED_TMUX_BIN), so this NEVER touches your real tmux sessions.
#
...
#!/bin/sh
clear
printf '=== OFFHOOK E2E  mosh+tmux  (scroll up to see lower numbers) ===\n'
i=1
while [ $i -le 60 ]; do
...
DERIVED="$(mktemp -d)"
xcodebuild -project "$REPO_ROOT/Offhook.xcodeproj" -scheme Offhook -configuration Debug \
...
xcrun simctl launch "$SIM_UDID" "$BUNDLE_ID" \
  ...
  -OFFHOOK_SEED_HOST "$SSH_HOST" -OFFHOOK_SEED_PORT "$SSH_PORT" \
  -OFFHOOK_SEED_MOSH 1 -OFFHOOK_SEED_MOSH_BIN "$MOSH_SERVER" \
  -OFFHOOK_SEED_TMUX 1 -OFFHOOK_SEED_TMUX_BIN "$WRAP" >/dev/null
