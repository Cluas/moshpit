#!/usr/bin/env bash
# Archive and export a SIGNED App Store build → build/AppStore/Ringdown.ipa
#
...
ARCHIVE="build/Ringdown.xcarchive"
...
# that is what makes the app and the RingdownIsland extension come out with the
# identical CFBundleVersion that App Store validation insists on. Override with
# RINGDOWN_BUILD=n to re-upload after a rejection without a dummy commit.
BUILD="${RINGDOWN_BUILD:-$(git rev-list --count HEAD)}"
...
if [ project.yml -nt Ringdown.xcodeproj/project.pbxproj ]; then
  echo "✘ project.yml is newer than Ringdown.xcodeproj — run: xcodegen generate" >&2
  exit 1
fi
...
APP="$ARCHIVE/Products/Applications/Ringdown.app"
...
echo "✓ $EXPORT_DIR/Ringdown.ipa  ($(du -h "$EXPORT_DIR/Ringdown.ipa" | cut -f1))"
echo
echo "  Upload with either:"
echo "    open $ARCHIVE          # Xcode Organizer ▸ Distribute App"
echo "    xcrun altool --upload-app -f $EXPORT_DIR/Ringdown.ipa -t ios \\"
