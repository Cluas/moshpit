...
### 1. 单元回放（CI 常驻）— `Tests/MoshpitTests/Services/TmuxSessionControllerTests.swift`
...
```bash
python3 scripts/spikes/tmux-cc-lab/lab.py copymode      # copy-mode 对 -CC 可见性
python3 scripts/spikes/tmux-cc-lab/lab.py wheel         # 滚轮 diff 重绘基线
python3 scripts/spikes/tmux-cc-lab/lab.py sizewar       # 尺寸战争全录（release→敌人赢→repin）
python3 scripts/spikes/tmux-cc-lab/lab.py capturerace   # 回宽瞬间 capture 的陈旧度
python3 scripts/spikes/tmux-cc-lab/lab.py analyze <bin> # 分析任意 -CC 字节流录像
```
...
```bash
./scripts/spikes/tmux-cc-lab/run-e2e.sh [sim-udid]   # 无 lab 时测试自动 skip
```
