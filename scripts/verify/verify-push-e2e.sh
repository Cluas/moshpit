...
#
# Usage:  scripts/verify/verify-push-e2e.sh
set -eu
...
MOSHPIT_PUSH_CONF="$TMP/push.conf" MOSHPIT_PUSH_HOST=m1-pro TMUX_PANE=%3 TMUX= \
  sh "$ROOT/scripts/host/moshpit-push.sh" attention claude "$TITLE" \
...
MOSHPIT_PUSH_CONF="$TMP/push.conf" MOSHPIT_PUSH_HOST=m1-pro TMUX_PANE=%3 TMUX= \
  sh "$ROOT/scripts/host/moshpit-push.sh" working claude "reading a file"
...
OUT=$(MOSHPIT_PUSH_CONF="$TMP/absent.conf" sh "$ROOT/scripts/host/moshpit-push.sh" attention claude x 2>&1)
...
MOSHPIT_PUSH_CONF="$TMP/push.conf" MOSHPIT_PUSH_HOST=m1-pro TMUX_PANE=%3 TMUX= \
  sh "$ROOT/scripts/host/moshpit-push.sh" done claude "finished" || fail "sender failed"
AFTER=$(grep -c "DRY RUN push" "$TMP/relay.log")
...
sleep 3
MOSHPIT_PUSH_CONF="$TMP/push.conf" MOSHPIT_PUSH_HOST=m1-pro TMUX_PANE=%3 TMUX= \
  sh "$ROOT/scripts/host/moshpit-push.sh" --test "selftest-abc123" || fail "self-test failed"
...
for name, rel in [("stamp", "scripts/host/moshpit-stamp.sh"), ("sender", "scripts/host/moshpit-push.sh")]:
    marker = f'    static let {name} = #"""\n'
    start = swift.index(marker) + len(marker)
    end = swift.index('\n"""#', start)
    if swift[start:end] != (root / rel).read_text().rstrip("\n"):
        sys.exit(f"HostScripts.{name} has drifted - run scripts/gen/gen-host-scripts.py")
