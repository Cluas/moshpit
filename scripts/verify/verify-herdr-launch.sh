...
# "WHERE YOUR PRIVATE KEY GOES" note in scripts/verify/smoke-localhost.sh, and prefer
# a throwaway key via MOSHPIT_SSH_KEY.
...
# would. `--stop-server` at the end shuts it down; pass MOSHPIT_KEEP_SERVER=1
...
# MOSHPIT_COLD=1 tests the other scenario: herdr installed but NO server
...
# scenario under test.
if [ "${MOSHPIT_COLD:-0}" != "1" ] && ! timeout 5 herdr api snapshot >/dev/null 2>&1; then
...
if [ "${MOSHPIT_COLD:-0}" != "1" ] && [ "$EMPTY" = "yes" ]; then
...
# MOSHPIT_MOSH=1 runs the same check over mosh. Both transports render herdr's
...
# an earlier bundle id had long since been granted on this simulator, so the
