...
#
# Tests/MoshpitTests/Services/MoshSwitchReplayTests.swift replays the .bin files
...
swiftc -O -parse-as-library -o "$BIN" \
  "$REPO_ROOT/scripts/capture/capture_mosh_switch_bytes.swift" \
  "$REPO_ROOT/scripts/spikes/spike_mosh_shims.swift" \
...
python3 "$REPO_ROOT/scripts/gen/gen_mosh_switch_content.py" \
