...
/// agent Live Activity (see Ringdown/Island/AgentActivityMonitor.swift). One
/// activity lists every active agent; the pill shows the most-urgent one.
@main
struct RingdownIslandBundle: WidgetBundle {
    var body: some Widget {
        RingdownIslandLiveActivity()
        RingdownStatusWidget()   // home- / lock-screen agent status (App Group pull)
    }
}

private let teal = Color(red: 0.37, green: 0.89, blue: 0.85)
private let amber = Color(red: 1.0, green: 0.62, blue: 0.04)

struct RingdownIslandLiveActivity: Widget {
            ...
            return DynamicIsland {
                DynamicIslandExpandedRegion(.leading) {
                    HStack(spacing: 6) {
                        ...
                        Text(headline.map { displayCommand($0) } ?? "Ringdown")
...
private struct AgentRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            // What the agent is doing / asking (hook @ringdown_title) — amber when
    ...
    var body: some View {
        HStack(spacing: 4) {
            Image(systemName: "pause.circle")
                .font(.system(size: 10))
            Text("paused — open Ringdown to refresh")
                .font(.system(size: 10, design: .monospaced))
        }
        .foregroundStyle(.white.opacity(0.45))
    }
...
private func stateColor(_ state: AgentActivityAttributes.AgentState) -> Color {
    ...
    case .done:      return Color(red: 0.55, green: 0.85, blue: 0.55)   // soft green
