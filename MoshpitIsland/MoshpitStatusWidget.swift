...
/// Home- and lock-screen Widget showing live Moshpit agent status. Unlike the Live
...
private let wTeal = AgentPalette.working
private let wAmber = AgentPalette.attention
private let wGreen = AgentPalette.done
...
struct MoshpitStatusWidget: Widget {
    var body: some WidgetConfiguration {
        StaticConfiguration(kind: "MoshpitAgentStatus", provider: AgentStatusProvider()) { entry in
        ...
        .description("Live status of your Moshpit agents.")
...
struct AgentStatusWidgetView: View {
    ...
    private var summaryLine: String {
        ...
        if isStale { return "paused — open Moshpit" }
    ...
    private var small: some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack(spacing: 5) {
                ...
                Text("Moshpit").font(.system(size: 12, weight: .bold)).foregroundStyle(.white)
                Spacer()
    ...
    private var rectangular: some View {
        VStack(alignment: .leading, spacing: 2) {
            HStack(spacing: 5) {
                ...
                Text(headline.map { $0.command.isEmpty ? "shell" : $0.command } ?? "Moshpit")
    ...
    @ViewBuilder private var inline: some View {
        if state.attentionCount > 0 {
            ...
            Label("Moshpit · no agents", systemImage: "circle")
        }
    }
}
