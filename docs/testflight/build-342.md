...
sent nothing, so nudging the cursor one column meant pressing the ✛ and pushing
...
swiping the app away can *start* on the ✛ — and that used to type a real `←` on
...
1. **Nudge the ✛ a couple of millimetres and let go.** One arrow, immediately —
...
5. **Swipe the app away, starting your swipe right on the ✛.** Nothing should be
...
- **`←↑↓→` "Tap arrows"** — four arrow keys in one control: tap sends one arrow,
  hold repeats it. A tap on the ✛ can never send a direction (a tap has no
