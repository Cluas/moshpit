...
```
主机进程:  claude … server:gitlab    cwd=~/code/sample-app   ← 在 Terminal 里起的
...
{"type":"worktree_created",
 ...
 "root_pane":{"pane_id":"w4:p1","cwd":"/Users/dev/.herdr/worktrees/repo/moshpit-probe"}}
```
...
w4:p2  cwd=/Users/dev/code/moshpit                  ← agent start 建的,错的
```
...
* `HomeView` 树的区标题写死 `SESSIONS` → 改走 vocabulary
...
| `UI/Home/HomeView.swift` ✅ | `agentsSection` + 纯函数 `agentEntries(snapshot:hooks:)`;区标题与统计芯片改走 vocabulary |
...
2. **状态语义色跟着主题走,且两个表面已经分叉。** `AgentSignal.working` 用的是
   ...
   收进 MoshpitKit 的 **`Island/AgentPalette.swift`**(App 和 island 都链接这个 package),
...
7. **杂项打磨。** `SheetListRow` 状态点带 VoiceOver 文字(色弱/读屏不再只有颜色);两个以上
   ...
   Prompt 多行(1–4 行生长);repo 菜单副标题 `~/code/moshpit` 缩写(`AgentTaskRequest.abbreviatePath`);
