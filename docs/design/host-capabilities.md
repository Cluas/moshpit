...
Moshpit 假设目标机已装 tmux(会话持久化/原生导航)和 mosh-server(漫游)。
现实中大量主机两者皆无。当前的失败形态:
...
3. **不偷装**:任何安装动作必须用户显式触发,且**在终端里可见地执行**(sudo 密码、输出全程可见),Moshpit 只代填命令。
...
- 新:`Moshpit/Services/HostCapabilities.swift`(探测 + 缓存,~80 行)
- 改:`SessionHub.start/startMosh`(探测注入 + 降级分支,~40 行)
- 新:`Moshpit/UI/Terminal/HostBannerView.swift` + `InstallAssistSheet.swift`(~150 行)
- 改:`TmuxEmptyStateView` 分支、`ConnectionCard` 降级 pill
- 测试:HostCapabilities 解析单测(mock exec 输出);UI 测试用 `-MOSHPIT_SEED_TMUX_BIN /bin/false` 模拟缺 tmux 走降级路径
...
- 在 Moshpit 里内置二进制分发(签名/架构矩阵成本,v2 再议 static-binary 助手)
