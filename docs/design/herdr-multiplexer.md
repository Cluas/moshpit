...
| `UI/Terminal/TerminalScrollGesture.swift` | herdr 模式下手势直接翻译成 `terminal.scroll`,跳过 `mouse_any_flag` 启发式 |
| `Services/Island/AgentActivityMonitor.swift` | herdr 模式从 `agent_status` 直接构 `AgentHook`(`working/blocked/done` → 现有三态),`state_change_seq` 当变更序号 |
| `UI/Island/IslandHooksInstallView.swift` | herdr 连接下整页隐藏 —— 不需要装任何东西 |
| `UI/Tmux/TmuxSheets.swift` / `HomeView.swift` | 泛型已经是 `<C: TmuxControlling>`,换协议名即可;文案里的 "tmux" 改成多路复用器中性词 |
...
| `scripts/verify/verify-herdr-launch.sh` | 实机端到端验证(SSH,`MOSHPIT_MOSH=1` 走 mosh) |

验收结果:

* 单测 364 全过,新增 17 条 —— 老格式 JSON 迁移、herdr 探测、安装命令合成、启动命令。
* `scripts/verify/verify-herdr-launch.sh` 干净重装后跑通:SSH 与 mosh 两条传输都起了 herdr TUI,
...
* 单测 389 全过,新增 25 条 —— 解码器吃**真实 0.7.3 抓包**(`Tests/MoshpitTests/Fixtures/herdr-snapshot.json`)、版本容差、shell 注入引号、轮询失败保留旧树。
...
* `scripts/verify/verify-herdr-launch.sh` 现在同时断言"服务端活着"和"App 读到了树"。
...
* `scripts/verify/verify-herdr-launch.sh` 现在断言四件事:服务端活着、App 读到树、**窗格是整屏宽(没有侧边栏)**、**打字到达窗格**、**切窗格后打字仍然到达**。
...
| `Services/Island/AgentActivityMonitor.swift` | `ConnRef.controller` 改存协议存在类型;`track` 泛型化 |
