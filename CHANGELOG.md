...
- **APP filing number in Settings, for the mainland store.** The footer under
  the build identity shows 浙ICP备2025212603号-2A and opens the MIIT filing
  portal on tap. China's regulator requires the number inside the app as well
  as on the store listing. It appears only for copies bought from the China
  mainland App Store (StoreKit's storefront, with the device region as the
  fallback when StoreKit has no answer); everyone else's footer is unchanged.
...
- **Opening Settings or Add Connection no longer crashes the app on a Mac.**
  Run on Apple silicon as "Designed for iPad", SwiftUI builds a sheet's content
  in a host that does not inherit the presenting view's environment — iPhone
  and iPad inherit fine — so the first screen inside any sheet to ask for an
  app-wide object (settings, the theme store, the key store) tripped an
  assertion and took the app down; on a fresh Mac install that was the first
  click. Every sheet, cover and popover now hands the app's objects to what it
  presents explicitly, so the lookup no longer depends on the host.
- **One Dynamic Island, not one per relaunch.** A Live Activity belongs to the
  system and outlives the app, but the app only remembered the one it had
  started in the current process: after a relaunch — a crash, iOS reclaiming
  memory, an update — it requested a fresh island and the old one lingered,
  frozen at "paused", for up to eight hours. Three launches, three identical
  islands. The app now adopts an island it finds already running and ends any
  extras, and ends all of them when no agent is left to show.
