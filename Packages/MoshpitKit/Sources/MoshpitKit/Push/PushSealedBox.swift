...
/// implemented three times — there, in `scripts/host/moshpit-push.sh`, and here. All
/// three are pinned to one frozen vector (`scripts/verify/push-vector.sh`); if you
...
public enum PushSealedBox {
    ...
    public struct Envelope: Codable, Equatable {
        ...
        public init(v: Int, iv: String, ct: String, mac: String) {
            self.v = v; self.iv = iv; self.ct = ct; self.mac = mac
        }
    }
    ...
    public struct Status: Codable, Equatable {
        ...
        public init(conn: String, host: String, sess: String? = nil, pane: String, agent: String? = nil,
                    state: String, title: String? = nil, ts: Int, dur: Int? = nil) {
            self.conn = conn; self.host = host; self.sess = sess; self.pane = pane; self.agent = agent
            self.state = state; self.title = title; self.ts = ts; self.dur = dur
        }
    }
    ...
    public static func open(_ envelope: Envelope, secretHex: String) throws -> Status {
    ...
    public static func openRaw(_ envelope: Envelope, secretHex: String) throws -> Data {
    ...
    public static func seal(_ plaintext: Data, secretHex: String, ivHex: String? = nil) throws -> Envelope {
        let keys = try deriveKeys(secretHex: secretHex)
        let iv: Data
        if let ivHex {
    ...
    public struct Keys: Equatable {
        public let encHex: String
        public let macHex: String

        public init(encHex: String, macHex: String) {
            self.encHex = encHex; self.macHex = macHex
        }
    }
    ...
    public static func deriveKeys(secretHex: String) throws -> Keys {
...
extension Data {
    ...
    public var moshpitHexString: String {
    ...
    public init?(moshpitHex hex: String) {
