...
/// ```
///
/// Lives in MoshpitKit so the extensions log under the same subsystem and
/// categories as the app; `log stream` then shows one conversation.
public enum Log {
    ...
    public static let mosh = Logger(subsystem: subsystem, category: "mosh")
