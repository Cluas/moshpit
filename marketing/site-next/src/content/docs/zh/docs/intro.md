...
实时的 agent 状态只在 Moshpit 处于前台、或刚进后台的短时间内更新——iOS 一挂起连接，Live Activity 就停在它最后看到的样子（并且会承认「paused」），直到你回来。能跨过这个缺口的是推送：你自己的主机把一条只有你手机能解开的提醒密封好、经 Moshpit 的中转发出，所以「agent 在等你」照样能落到关着 App 的锁屏上。完整链路见[智能体通知，端到端](/zh/docs/agents)。
