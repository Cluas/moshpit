...
- **把手机上的 VPN 关掉**再连。分流模式的 VPN 配置是最常见的元凶。
...
没有推送服务器——这是「中间什么都不放」换来的代价。 通知是在设备本地从活着的会话里生成的。它们只在 Moshpit 处于前台、 或刚进后台不久时才会触发；**完全被挂起的 App 不会去轮询**， 所以 agent 状态在那期间停止更新，等你回来才补上。
