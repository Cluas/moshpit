...
struct HomeView: View {
    ...
    @State private var showSettings = CaptureScreen.requested.map { $0 != .addConnection } ?? false
    @State private var showAddConnection = CaptureScreen.requested == .addConnection
    #else
    ...
    @State private var splitIsPortrait = false
    ...
    var body: some View {
        ...
        .appSheet(isPresented: $showSettings) {
            SettingsScreen(liveSession: liveSessionForHooks)
        }
        .appSheet(isPresented: $showAddConnection) {
            AddConnectionView(store: store, keychain: keychain)
        }
        .appSheet(item: $editingConnection) { connection in
    ...
    /// Its own property, not inline in `body`: the modal chain below is long,
    /// and an `if` at its root was one expression too many for the type
    /// checker.
    @ViewBuilder private var layout: some View {
        if usesSplitLayout { splitLayout } else { stackLayout }
    }

    /// iPad at regular width: the connection list and the terminal side by
    /// side. An iPhone — including a Max in landscape, which is also regular
    /// — keeps the stack: a landscape phone is for a wider terminal, not a
    /// sidebar, and swapping hierarchies on every rotation would re-host the
    /// terminal each time.
    private var usesSplitLayout: Bool {
        UIDevice.current.userInterfaceIdiom == .pad && horizontalSizeClass == .regular
    }

    /// The phone: Home pushes the terminal.
        ...
        NavigationStack(path: $path) {
            ...
            .navigationDestination(for: UUID.self) { id in
                if let connection = store.connections.first(where: { $0.id == id }) {
                    ...
                    // Plain push, not `.navigationTransition(.zoom)`: the zoom
                    // transition installs its own drag-to-dismiss on the pushed
                    // view, and a downward swipe in the terminal — a scroll —
                    // popped the whole screen (iOS 26.2 simulator, 2026-09-11).
                    // SwiftUI has no way to veto that gesture, so the terminal
                    // keeps the standard push.
    ...
    /// The iPad: Home is the sidebar, the terminal the detail. `path` is the
    /// one source of truth for both layouts, so an open terminal survives the
    /// layout changing under it (a Split View resize) — the session lives in
    /// the hub and re-hosting is idempotent.
    private var splitLayout: some View {
        NavigationSplitView(columnVisibility: $columnVisibility) {
            homeList
                // Wider than the stock sidebar: a card carries a name, a
                // transport chip and a user@host, and at 320pt the host wraps.
            ...
            if let id = path.last,
               let connection = store.connections.first(where: { $0.id == id }) {
                TerminalScreen(connection: connection, hub: hub,
                    ...
                    // A different host is a different screen: without this the
                    // struct is reused and keeps the first host's state.
                    .id(id)
            } else {
                ContentUnavailableView {
                    Label("No Terminal Open", systemImage: "terminal")
                } description: {
                    Text("Pick a host from the sidebar, or add one with ⌘N.")
                }
                .background { HomeStage() }
            }
        }
    ...
    /// The connection list with its title and toolbar — the root of the
    /// stack on a phone, the sidebar on an iPad.
    private var homeList: some View {
        List {
            if !store.connections.isEmpty {
                // One status row instead of three decorative counters — it
                // says the one thing that matters right now and TAKES YOU
                // THERE: an agent waiting outranks everything, then live
                // connections; all-quiet is just a quiet line.
                Section {
                    statusRow
                        .listRowBackground(statusRowFill)
                        .listRowSeparatorTint(Ink.hairline)
                } header: {
                    // The tagline the brand card used to carry, in the same
                    // small mono voice, now labelling the one status line.
        ...
        // Only iPad and iPhone landscape ever reach the cap.
    ...
    /// Show a connection's terminal: a push on the phone, the detail column
    /// on the iPad. Reassigning `path` to its current value is a no-op for
    /// NavigationStack, so the check is only for clarity.
    private func open(_ id: UUID) {
        if path != [id] { path = [id] }
        // In portrait the sidebar overlays the terminal; get out of its way.
        if usesSplitLayout && splitIsPortrait { columnVisibility = .detailOnly }
    }

    /// Back/⌘W from the detail: drop the terminal and bring the list back —
    /// in portrait the sidebar was tucked away on open, and a placeholder
    /// with no way to the hosts is not "home".
    ...
    /// The status line: what is waiting on you, and where to go for it.
    private var statusRow: some View {
        let attention = attentionAgents.count
        let live = attention > 0 || liveCount > 0
        return Group {
            if live {
                Button {
                    ...
                    HStack(spacing: 12) {
                        statusLabel(attention: attention, live: true)
                        Spacer(minLength: 8)
                        MiniChevron()
                    }
                }
                ...
                statusLabel(attention: 0, live: false)
                    .accessibilityIdentifier("home-status")
            }
        }
    }

    /// Mono, tracked, dim unless something is live: the status strip's voice
    /// from the brand card, kept when the card became a list row.
    private func statusLabel(attention: Int, live: Bool) -> some View {
        HStack(spacing: 10) {
            Image(systemName: statusIcon(attention: attention))
                .font(.system(.caption, weight: .semibold))
                .foregroundStyle(statusTint(attention: attention))
                .frame(width: 20)
            Text(verbatim: statusText(attention: attention))
                .font(Face.mono(11, .semibold))
                .kerning(0.7)
                .foregroundStyle(live ? Ink.secondary : Ink.meta)
                .multilineTextAlignment(.leading)
                .fixedSize(horizontal: false, vertical: true)
        }
    }

    /// The row's fill says the same thing its icon does: amber wash when an
    /// agent is waiting, accent wash when something is live, ink otherwise.
    private var statusRowFill: Color {
        // swiftlint:disable:next empty_count - `count` is a tuple field, not a collection
        if attentionAgents.count > 0 { return Ink.warn.opacity(0.10) }
        if liveCount > 0 { return Ink.accent.opacity(0.08) }
        return Ink.group
    }

    private func statusIcon(attention: Int) -> String {
        if attention > 0 { return "sparkle" }
        if liveCount > 0 { return "bolt.fill" }
        return "moon.zzz"
    }

    private func statusTint(attention: Int) -> Color {
        if attention > 0 { return Ink.warn }
        if liveCount > 0 { return Ink.accent }
        return Ink.meta
    }

    private func statusText(attention: Int) -> String {
        if attention > 0 {
            return attention == 1
                ? String(localized: "1 agent needs you")
                : String(localized: "\(attention) agents need you")
        }
        if liveCount > 0 {
            return liveCount == 1
        ...
        let saved = store.connections.count
        return saved == 1
    ...
    /// the button reads as broken and invites a second tap, so the card shows
    /// a spinner for the duration and ignores repeats.
    private func disconnect(_ connection: ServerConnection) {
    ...
    /// Same teardown as `disconnect`, chained straight into a fresh connect()
    /// — a stalled tmux attach leaves SSH `.connected`, so a plain retry
    /// (onConnect alone) would no-op.
    private func retryAttach(_ connection: ServerConnection) {
        guard !disconnectingIds.contains(connection.id) else { return }
        disconnectingIds.insert(connection.id)
        Haptics.tap()
        Task {
    ...
    private var emptyState: some View {
        ContentUnavailableView {
            Label {
                ...
                // The app's own mark where the system would put a glyph — the
                // empty pit is still the pit.
                AppIconMark(size: 72)
                    .padding(.bottom, 6)
            }
        } description: {
            Text("Add a host to keep your agents within reach. Moshpit keeps sessions alive across Wi-Fi / 5G handoff.")
        } actions: {
            Button {
                handleAdd()
            } label: {
            ...
            .tint(Ink.accent)
            .accessibilityIdentifier("home-add-empty")
        }
    }
}

/// Version line closing the connections list. Prefers the
/// `MoshpitBuildStamp` written by scripts/release/build-ipa.sh (git SHA + time, "+"
...
/// What the home list stands on: the theme's screen colour under the signal
/// grid — the stage the connecting screen shares, so the app has one floor.
/// Forms stay flat; this is the front door.
...
struct ConnectionCard: View {
    ...
    @ViewBuilder private var hostHead: some View {
        // At accessibility sizes the head stacks: avatar, then name, then the
        // status line, each on its own row. Side by side, a 42pt avatar and a
        // 9pt SAVED tag next to 30pt text read as a layout accident.
        let stacked = dynamicTypeSize.isAccessibilitySize
        let headLayout = stacked
            ? AnyLayout(VStackLayout(alignment: .leading, spacing: 10))
            : AnyLayout(HStackLayout(spacing: 13))
        let statusLayout = stacked
            ? AnyLayout(VStackLayout(alignment: .leading, spacing: 4))
            : AnyLayout(HStackLayout(spacing: 7))
        HStack(alignment: stacked ? .top : .center, spacing: 4) {
                ...
                headLayout {
                    ZStack(alignment: .bottomTrailing) {
                        ...
                        Circle()
                            ...
                            .overlay(Circle().strokeBorder(Self.surface, lineWidth: 2))
                            .offset(x: 2, y: 2)
                    }
                    ...
                    if !isLive {
                        // The head's tap connects; say so.
                        Label("Connect", systemImage: "power")
                            .labelStyle(.iconOnly)
                            .font(.footnote.weight(.bold))
                            .foregroundStyle(Ink.accent)
                            .frame(width: 32, height: 32)
                            .background(Ink.accent.opacity(0.12), in: Circle())
                    }
                }
            ...
            if isDisconnecting {
                ...
                // Visible, not just a long-press away: a saved host's Edit and
                // Delete used to hide behind the context menu.
                Menu {
                    cardActions
                } label: {
                ...
                .accessibilityIdentifier("card-menu-\(connection.displayName)")
            }
        }
        .padding(EdgeInsets(top: 14, leading: 15, bottom: 13, trailing: 8))
    }
