...
/// ``AppTheme``) and each bakes in its own fill colour, so the gallery offers
/// real alternatives — different figures — rather than recolors of one mark.
struct AppIconOption: Identifiable, Equatable {
    /// Stable id used in settings. "default" is the primary icon.
    let id: String
    let name: String
    /// The name actool compiled the Icon Composer document under (and wrote
    /// into `CFBundleAlternateIcons`), or nil for the primary icon — exactly
    /// what `setAlternateIconName` wants.
    let alternateName: String?
    /// Bundled preview PNG, rendered from the document by Icon Composer's
    /// ictool the way SpringBoard draws it (scripts/gen/generate-alternate-icons.mjs).
    /// Icon documents have no image `UIImage(named:)` can load, so the picker
    /// shows these instead.
...
enum AppIconCatalog {
    // An easter-egg gallery in the primary's own Liquid Glass language: every
    // alternate is its own figure on its own fill — recolors of one mark all
    // read the same at a glance, which is the feedback that retired the old
    // Teal/Green/Amber/Daylight/Mono set. Each figure carries one colour, so
    // the range survived.
    static let all: [AppIconOption] = [
        AppIconOption(id: "default", name: "Moshpit", alternateName: nil,
                      previewAsset: "IconPreview-Moshpit"),
        // The crowd-surf mark that shipped as primary in 1.0.0–1.0.2. Retired
        // when the panes icon took over (the crowd read as an ellipsis at
        // 60pt), kept so nobody loses the icon they bought the app with.
        AppIconOption(id: "pit", name: "Pit", alternateName: "AppIcon-Pit",
                      previewAsset: "IconPreview-Pit"),
        // The Dynamic Island with an agent's status in it.
        AppIconOption(id: "island", name: "Island", alternateName: "AppIcon-Island",
                      previewAsset: "IconPreview-Island"),
        AppIconOption(id: "wq", name: ":wq", alternateName: "AppIcon-Wq",
                      previewAsset: "IconPreview-Wq"),
        AppIconOption(id: "prefix", name: "Prefix", alternateName: "AppIcon-Prefix",
                      previewAsset: "IconPreview-Prefix"),
        AppIconOption(id: "localhost", name: "Localhost", alternateName: "AppIcon-Localhost",
                      previewAsset: "IconPreview-Localhost"),
        // A prompt waiting, in the amber the app uses for "an agent needs you".
        AppIconOption(id: "prompt", name: "Prompt", alternateName: "AppIcon-Prompt",
                      previewAsset: "IconPreview-Prompt"),
        AppIconOption(id: "cursor", name: "Cursor", alternateName: "AppIcon-Cursor",
                      previewAsset: "IconPreview-Cursor"),
        AppIconOption(id: "hail", name: "Hail", alternateName: "AppIcon-Hail",
                      previewAsset: "IconPreview-Hail"),
    ]
