...
struct AccentGalleryView: View {
    ...
    var body: some View {
        ...
        .sensoryFeedback(.selection, trigger: pendingID)
...
private struct AccentRow: View {
    ...
    var body: some View {
        // At accessibility sizes the hex code moves under the name instead of
        // squeezing it into "Signal…" beside a value nobody needs to read first.
        let stacked = dynamicTypeSize.isAccessibilitySize
        let textLayout = stacked
        ...
        HStack(spacing: 11) {
            Button(action: onSelect) {
                HStack(spacing: 12) {
                    ...
                    textLayout {
                        Text(theme.name).font(.body).foregroundStyle(Ink.primary)
