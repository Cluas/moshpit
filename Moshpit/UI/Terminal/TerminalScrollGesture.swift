...
/// row-only routes fold it into rows with the remainder carried. The lift
/// settles the scroll where the finger left it: no coast. A UIScrollView-style
/// fling shipped in 1.0.4 (399) and came out on device feedback — over a
/// terminal, content that keeps moving after the finger stops reads as
/// overshoot, not ease ("惯性可以去掉吗 体验不是很好"). The version before
/// either quantised to whole rows per event and threw the remainder away, so
/// the content lurched under the finger.
