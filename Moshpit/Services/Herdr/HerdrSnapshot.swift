...
/// against a real captured snapshot (`Tests/MoshpitTests/Fixtures/herdr-snapshot.json`).
///
/// ### The mapping
///
...
enum HerdrSnapshot {
    ...
    static func firstJSONObject(in raw: String) -> [String: Any]? {
        ...
        for index in start..<scalars.count {
            ...
            if inString {
                if byte == UInt8(ascii: "\\") { escaped = true } else if byte == UInt8(ascii: "\"") { inString = false }
                continue
            }
            switch byte {
